...
def opinion_extractor(aspect_token, parsed_sentence):
    ...
    tokens = [token for token in parsed_sentence.get_dependants(aspect_token)]
    for token in tokens:
        if token.deprel == "amod":
            dependants = parsed_sentence.get_dependants(token)
            for dependant in dependants:
                if dependant.deprel == "advmod":
                    #opinions.append(dependant.form)
                    #opinions.append(token.form)
                    words = [dependant.form, token.form]
                    joined_string = "-".join(words)
                    opinions.append(joined_string)

    #opinions += [dependant.form for dependant in parsed_sentence.get_dependants(aspect_token) if dependant.deprel == "amod"]
...
save_file_path = r"\\smbhome.uscs.susx.ac.uk\jw468\Documents\Course Modules\Year 2\Natural Language Engineering\Lab 8\output3.txt"    # Set this to the location of the file you wish to create/overwrite with the saved output.
